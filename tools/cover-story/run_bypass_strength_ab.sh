...
common=(
  --server "$COMFY_SERVER"
  --mode performers-v6
  --label performers-v18-bypass-ab
  ...
  --age-wording band
  --seed 2026072700
  --download-dir /mnt/Misc/sd/cover-story/experiments/performers-v18-bypass-ab
)

for variant in "${variants[@]}"; do common+=(--variant "$variant"); done

python3 tools/cover-story/experiment_headshots.py "${common[@]}" --bypass-strength 2 --suffix A "$@"
python3 tools/cover-story/experiment_headshots.py "${common[@]}" --bypass-strength 1 --suffix B "$@"
