...
common=(
  ...
  --age-wording band
  --seed 2026072700
  --download-dir /mnt/Misc/sd/cover-story/experiments/performers-v20-cfg-ab
)

for variant in "${variants[@]}"; do common+=(--variant "$variant"); done

python3 tools/cover-story/experiment_headshots.py "${common[@]}" --cfg 1 --suffix A "$@"
python3 tools/cover-story/experiment_headshots.py "${common[@]}" --cfg 1.25 --suffix B "$@"
